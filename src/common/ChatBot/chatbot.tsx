    ...
    <div className="fixed bottom-2 right-2">
      ...
      >
        <div className="bg-primary text-white px-4 py-2 flex justify-between rounded-t-[25px] items-center">
          <span className="ml-20" aria-label="Chat with us">Chat with us!</span>
          <button onClick={() => setIsOpen(false)} aria-label="Close Chatbot" className="text-xl font-bold cursor-pointer">x</button>
        </div>
        <div className="p-3 h-60 overflow-auto">
          {messages.map((msg, index) => (
            <div key={index} aria-label={`Message from ${msg.sender}: ${msg.text}`} className={`mb-2 p-2 rounded ${msg.sender === "bot" ? "bg-gray-200 text-left mr-8" : "bg-green-100 text-right ml-8 self-end"}`}>
              {msg.text}
            </div>
          ))}
        </div>
        <div className="border-gray-200 border-t flex">
          ...
          />
          <button onClick={sendMessage} aria-label="Send" className="bg-primary text-white px-3 rounded-r-[25px] ml-2">Send</button>
        </div>
      </div>
      }
      ...
      {!isOpen && (
        <button
          onClick={() => setIsOpen(true)} aria-label="Start Chatbot"
